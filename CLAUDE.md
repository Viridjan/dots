...
niri/.config/niri/
  ...
  dms/                ← DankMaterialShell overrides; NOT stow-symlinked (copied via vjupdate --dms-export/--dms-import)
    ...
    outputs.kdl       (monitor config — auto-generated by DMS. NOT included in config.kdl since 2026-08-25;
                       fully inert, cfg/display.kdl replaced it. Left on disk because DMS keeps writing it.)
...
- **Do not** hand-edit `dms/colors.kdl` or `dms/outputs.kdl` *live* — DMS rewrites both within seconds of every session start, regardless of content, not just on theme/display changes. To change colors, edit the **repo** copy and run `vjupdate --dms-import` (pushes repo→live) + `niri msg action load-config-file`. `--dms-export` deliberately skips `colors.kdl` so DMS's regenerated version can't overwrite your curated one.
  ...
  **Why that matters (2026-08-25 audit):** `dms_export` used to blanket-`cp` every live `*.kdl` into the repo, including auto-generated ones. That silently reverted the repo's green focus-ring (`#4caf50`) and forest shadow (`#0d3320CC`) to DMS defaults in commit `ff2cd4e` (2026-06-17) — a commit whose message was about unrelated vjupdate changes — and it went unnoticed for two months while this file kept documenting green as current. `dms_export` is now restricted to `DMS_MANUAL`, and `dms_import` additionally pushes `DMS_AUTO` (`colors.kdl`) back live. Current colors are DMS-default purple; that's the deliberate choice as of 2026-08-25, not drift.
  ...
  `dms/outputs.kdl` needs no reapply either — it is **no longer included at all** (see Include order above), so DMS can keep rewriting it with zero effect.

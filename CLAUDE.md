...
niri/.config/niri/
  ...
  dms/                ← DankMaterialShell overrides; NOT stow-symlinked (copied via vjupdate --dms-export/--dms-import)
    ...
    binds.kdl         (keybinds — the live source)
    ...
    (outputs.kdl      — DELETED 2026-08-25. Was a full `output "DP-1"` block with unconditional VRR;
                       cfg/display.kdl replaces it. Deleted rather than left inert because it read as
                       authoritative, making "re-add the include" the obvious-but-wrong fix — which
                       silently reproduces the bug, since keyed nodes take the FIRST occurrence.
                       DMS did not recreate it within 20s of deletion in a running session; if it
                       reappears at some point, it is still harmless — nothing includes it.)
...
- **DMS overrides** — direction depends on whether DMS regenerates the file:
  - `DMS_MANUAL` (`alttab` `binds` `cursor` `layout` `windowrules` `wpblur`): edit **live** under `~/.config/niri/dms/`, then `vjupdate --dms-export` to copy into the repo and commit.
...
- **Do not** hand-edit `dms/colors.kdl` *live* — DMS regenerates it and your edit is lost. To change colors, edit the **repo** copy and run `vjupdate --dms-import` (pushes repo→live) + `niri msg action load-config-file`. `--dms-export` deliberately skips `colors.kdl` so DMS's regenerated version can't overwrite your curated one.
  ...
  **On DMS's regeneration timing:** the long-standing claim here was that DMS rewrites its auto-generated files "within seconds of every session start, regardless of content". Partly wrong — `dms/outputs.kdl` was deleted mid-session on 2026-08-25 and DMS had not recreated it 20s later. DMS most likely writes on display/theme *changes*, not on a timer. Treat `colors.kdl` as regenerated-at-some-point (hence repo-is-authoritative + `--dms-import`), but do not rely on a specific interval.
